...
@section('content')
    <section>
        <div class="section-body">
            <form class="form form-validate floating-label" action="{{route('unit.update',$unit->id)}}"
                  method="POST" enctype="multipart/form-data" >
            @method('PUT')
            @include('unit.form', ['header' => 'Edit unit <span class="text-primary">('.($unit->name).')</span>'])
            </form>
        </div>
    </section>
